...
func (c Column[V]) GetNameAsAliasWithTableAlias() string {
	if c.TableAlias == "" {
		if c.ColumnAlias == c.ColumnName {
			return c.GetName()
		}
	...
	if c.ColumnAlias == c.ColumnName {
		return fmt.Sprintf(`%s.%s`, c.TableAlias, c.GetName())
	}
	return fmt.Sprintf(`%s.%s AS %s`, c.TableAlias, c.GetName(), c.GetAlias())
}
...
type ColumnType interface {
	...
	GetNameAsAliasWithTableAlias() string
}
...
func (s Statement) GetNameAsAliasWithTableAlias() string {
	return s.SQL
}
