...
func TestSimpleSelectAs(t *testing.T) {
	query, err := Query(
		SELECT(
			UserTable.Password.SetAlias("pass"),
			UserTable.Email,
		),
		FROM(UserTable),
	)
	...
	if query.SQL != `SELECT "user"."password" AS "pass", "user"."email" FROM "user" AS "user";` {
		t.Errorf("Query is not correct: %s", query)
	}
...
// 		LEFT_JOIN(
// 			UserTable,
// 			EQUAL_COLUMNS(Use.UserID, UserTable.ID),
// 		),
// 	)
// 	if err != nil {
// 		t.Errorf("Error is not nil: %s", err)
...
func TestUnitSQLC_RAW(t *testing.T) {
	statemnt := SQLC_RAW(`-- name: GetOfficesListCount :one
	SELECT COUNT(*) FROM "office" LEFT JOIN area ON area.id = office.area_id
`)

	assert.Equal(t, `SELECT COUNT(*) FROM "office" LEFT JOIN area ON area.id = office.area_id`, statemnt.SQL)
}
